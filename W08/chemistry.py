def main():
    ...
    compute_molar_mass( parse_formula(formula, periodic_table) , periodic_table )
    return
...
def compute_molar_mass(symbol_quantity_list, periodic_table_dict):
    ...
    for element in symbol_quantity_list:
        symbol = element[0]
        quantity = element[1]
        result += round(periodic_table_dict[symbol][1] * quantity , 5)
    
    print( str(result) + ' grams/mole' )
    if result != 0 and __name__ == "__main__":
        print(str(round(mass / result, 5)) + ' moles')
